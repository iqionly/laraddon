...
class LaraddonServiceProvider extends ServiceProvider
{
    ...
    private function initClasses() {
        foreach ($this->deferClasses as $deferClass) {
            if (!class_exists($deferClass)) {
                throw new Exception("Class {$deferClass} does not exist.", 10000);
            }
            $class = $this->app->make($deferClass);
            ...
            foreach ($this->classes as $class) {
                try{
                    ...
                    if($class instanceof Initiable) {
                        ...
                        throw new \Exception("There some class is not Initiable.", 10001);
                    }
                } catch (\Exception $ex) {
                    if(!is_object($class)) {
    ...
    private function determineAddonsPath(): void
    {
        ...
        if($this->app->runningUnitTests() && is_string($unit_addon_path = env('PHPUNIT_ADDONS_PATH'))) {
            ...
            if($path === false) {
                throw new \Exception('Invalid addons path provided for PHPUnit tests.', 10002);
            }
            ...
            if(!is_string($path)) {
                throw new \Exception("Configuration 'laraddon.addons_path' must be a string.", 10003);
            }
            $this->addons_path = $path;
            unset($path);
        }
